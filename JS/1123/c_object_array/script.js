...
//: 배열의 크기는 같을 필요x, const 키워드로 사용 가능
...
// 객체에서 변수를 추출
// name 속성과 price 속성을 그대로 꺼내옴
const { name, price} = bookObject;
console.log(name, price);

// name속성을 aaa라는 이름, price 속성을 bbb라는 이름으로 출력
...
console.log(물건_1123);
console.log(물건_1124);
//: 얉은 복사(참조 복사)는 서로 이름만 다를 뿐 서로 같은 메모리 참조값을 가짐.
...
const puppyD = {
  예방접종: true,
  ...puppyA, //해당 코드가 예방접종 뒤로 갈 시 아래의 덮어쓰기가 되지 않고 puppyA의 값이 나온다.
  이름: '나비', // 기존의 속성을 덮어쓰기
  나이: 1 // 기존의 속성을 덮어쓰기
}
console.log(puppyD);