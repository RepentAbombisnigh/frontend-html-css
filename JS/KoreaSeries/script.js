...
document.getElementById('predict-button').addEventListener('click', function() {
  ...
  let gameOutcomes = [];
  // 시리즈가 끝날 때까지 진행 (한 팀이 4승을 하거나 최대 게임 수에 도달할 때)
  while (gamePlayed < maxGamges && results['LG' < 4 && results['KT'] < 4]) {
    // math.rando. 0과 1사이의 난수를 생성, 기반으로 승자를 결정
    // 0.5 미만일 경우 lg승리 그게 아니면 kt 승리로 처리
    ...
    gamePlayed++;
    // 경기 결과를 배열에 추가
    gameOutcomes.push(`${gamePlayed}차전 ${winner} 승리`);
  }

  //모든 게임이 종료된 경우 최종 결과를 화면에 표시
  displayResults(results);
});

//displayResults 함수는 최종 결과를 받아서 웹 페이지에 표시
