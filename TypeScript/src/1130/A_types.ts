...
// : 위의 형태는 JS의 '특수 내장 타입'이므로 극히 드물게 사용
// : 소문자 형태의 타입 사용을 권장
...
// 해당 형식에 4를 넣을 경우 string 형식이 아닌 number이기에 오류 발생
let list3: Array<number> = [4, 5, 6];
...
//! void 타입
// : 아무런 값이 없다
// : 주로 함수에서 반환값(return)이 없거나 반환값이 있더라도 반환하는 값이 없을 때 사용되는 타입
function voidType1(parameter: number): number {
  // 함수의 타입 정의
  // 파라미터와 반환값 정의가 가능
...
console.log(voidType1(3));

// ! null & undefined
// JS와 달리 TS에선 null과 defined의 경우 각각의 값만을 가짐
...
// null과 undefined는 모든 타입의 하위 타입이기 떄문에 두 타입은 모든 타입에 할당 가능

let nullNumber: number = 1;
// nullNumber = null; 
// : 원래 가능하나 현재 엄격한 null 검사로 인한 오류
// : tsconfig.json의 컴파일 옵션에 --strictNullChescks 컴파일 옵션으로 인하여 null과 undefined가 각각 해당 이름의 타입을 가진 변수에만 할당 가능
...
//: any 타입의 경우 사전에 타입 오류를 계산할 수 없기 때문에 예상치 못한 에러 발생 가능성 높음

// noImplicitAny
//: 타입이 지정되지 않은 값은 TypeScript 컴파일러가 자동으로 any 타입을 부여
//: TS 컴파일러가 noImplicitAny를 활성화하는 경우 암묵적으로 any로 간주하는 모든 경우에 오류 발생
function any(para: any) { // para: any가 아닌 para만 적는 경우 암묵적으로 any가 적용되지만 오류 방지를 위해 경고 발생
  // 
}
...
//! bigint
//: Es2020 이후 큰 정수를 다루기 위한 원시 타입
// BigInt 함수를 사용하여 bigint 값을 생성
// const oneHundred: bigint = BigInt(100);
// : error: 대상 라이브러리를 변경하려는 경우 'lib' 컴파일러 옵션을 'es2020' 이상으로 변경해 보세요

//! symbol
//: 전역적으로 고유한 참조값을 생성할 때 사용하는 타입
//: 유일하고 변경 불가능한 값으로 생성
//: Symbol() 함수로 생성

const referrence = Symbol('name'); // referrence에 name이라는 설명을 가진 심볼 생성
